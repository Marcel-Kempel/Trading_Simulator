    ...
    <div className="space-y-8">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-[14px] text-muted-foreground mt-0.5">
                {rangeSubtitle} · {formatEur(investmentGainLoss)}(
            ...
            <div className="flex items-center gap-2">
              ...
              <div
                className={`flex items-center gap-2 px-4 py-2 rounded-xl ${
                  isInvestmentPositive ? "bg-emerald-500/10" : "bg-red-500/10"
                }`}
              >
            ...
            <LineChart data={series}>
              ...
              <Tooltip
                ...
                formatter={(value: number, _name, item) => {
                  const gainLoss =
                    (item?.payload?.gainLoss as number | undefined) ?? 0;
                  return [
                    `${value >= 0 ? "+" : ""}${value.toFixed(2)}% (${formatEur(
                      gainLoss,
                    )})`,
                    "Investment Return",
                  ];
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200">
        <div className="p-8 border-b border-border">
          ...
          <p className="text-[14px] text-muted-foreground mt-0.5">
            Click a row to open stock performance
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-muted/30 backdrop-blur-sm">
              <tr>
                <th className="px-8 py-4 text-left text-[13px] font-semibold text-muted-foreground uppercase tracking-wider">
                  Symbol
                </th>
                <th className="px-8 py-4 text-left text-[13px] font-semibold text-muted-foreground uppercase tracking-wider">
                  Quantity
                ...
                <th className="px-8 py-4 text-right text-[13px] font-semibold text-muted-foreground uppercase tracking-wider">
                  P&L
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {positionsWithPrice.map((position) => (
                ...
                >
                  ...
                  <td className="px-8 py-5 whitespace-nowrap">
                    <span className="text-[15px] font-medium text-foreground">
                      {formatEur(position.avgPrice)}
                    </span>
                  </td>
                  <td className="px-8 py-5 whitespace-nowrap">
                    <span className="text-[15px] font-medium text-foreground">
                      {formatEur(position.currentPrice)}
                    </span>
                  </td>
  ...
  return (
    <div className="group bg-card border border-border rounded-2xl p-6 hover:shadow-lg hover:shadow-primary/5 transition-all duration-200 cursor-pointer">
      <div className="flex items-start justify-between mb-4">
        ...
        <div
          className={`p-2.5 rounded-xl ${bgClass} ${colorClass} group-hover:scale-110 transition-transform duration-200`}
        >
          {icon}
        </div>
      </div>
      <div className="space-y-2">
        <div className="text-[28px] font-semibold text-foreground tracking-tight">
          {value}
        </div>
